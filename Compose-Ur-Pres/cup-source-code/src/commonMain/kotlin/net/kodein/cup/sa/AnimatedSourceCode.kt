...
@Composable
private fun AnimatedSourceCodeContent(
    data: SAData,
    parts: List<TextPartNode>,
    sections: suspend (Int) -> List<ClassesSection>,
    step: Int,
    style: TextStyle,
    theme: SourceCodeTheme,
) {
    val partFractions = data.steps.mapIndexed { index, _ ->
        animateFloatAsState(
    ...
    Box {
        ...
        SelectionContainer(
            ...
            Text(
                text = data.fullText - data.hiddenRanges(step),
                style = style.copy(color = Color.Transparent),
                softWrap = false,
            )
        }

        val codeStyle by produceState(emptyList<StyleSection>(), step) { value = sections(step).applySourceCodeTheme(theme) }
...
public fun AnimatedSourceCode(
    ...
    AnimatedSourceCodeContent(
        data = data,
        parts = remember(data) {
            val lines = data.fullText.lineRanges()
                .filterNot { line -> data.blocks.any { it.range == line } }
                .map { SABlock(SABlock.ID.None, SABlock.Type.Lines, it) }

            (lines + data.blocks).asTree(
                range = SABlock::range,
                node = newNode@ { block, children ->
                    if (children.isEmpty()) {
                        return@newNode buildTextPartNode(block.range, data.fullText, block.id, emptyList())
                    }
                    val addedChildren = ArrayList<TextPartNode>()
                    var lastEnd = block.range.min
                    children.forEach { child ->
                        if (child.range.min != lastEnd) {
                            addedChildren += buildTextPartNode(TextRange(lastEnd, child.range.min), data.fullText)
                        }
                        lastEnd = child.range.max
                    }
                    if (lastEnd != block.range.max) {
                        addedChildren += buildTextPartNode(TextRange(lastEnd, block.range.max), data.fullText)
                    }
                    buildTextPartNode(block.range, data.fullText, block.id, (children + addedChildren).sorted())
                }
            )
        },
        sections = sections,
        step = min(max(0, step), data.steps.lastIndex),
