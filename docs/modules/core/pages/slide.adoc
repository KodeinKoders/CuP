...
WARNING: This is deprecated. You should use Compose Hot Reload to preview your content in real time.
...
[[context-data]]
== Attaching context data to a slide

You can attach custom context data to a `Slide` that can be consumed by your presentation decoration or by a plugin.

=== Attaching context data to a slide

Some plugins provide configuration `SlideContext` data classes that allow you to modify the behaviour of that plugin when the slide is displayed.

To attach a `SlideContext` to a slide, simply pass it to its `context` parameter:

[source, kotlin]
----
val whatever by Slide(
    context = SpeakerNotesMD("Amazing!")
) {}
----

You can combine multiple `SlideContextElement` into a single `SlideContext`:
...
----
val whatever by Slide(
    context = SpeakerNotesMD("Amazing!") + Export.only(1, 3) + AutoMovePause { 3.seconds }
) {}
----
* With the `slideContextOf` function:
+
[source, kotlin]
----
val whatever by Slide(
...
You can now attach this context data to a `Slide`:

[source, kotlin]
----
val whatever by Slide(
    context = Banner("Work In Progress!")
) {
    /* Slide content */
}
----

Once attached to a slide, xref:presentation.adoc#context-data[your presentation decoration can access this context].
