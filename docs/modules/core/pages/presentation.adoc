...
kotlin {
    sourceSets.commonMain.dependencies {
        ...
        implementation("net.kodein.cup:cup:{version}")

        // Optional: source code animation composables. <1>
        implementation("net.kodein.cup:cup-widgets-source-code:{version}")
        ...
        implementation("net.kodein.cup:cup-plugin-laser:{version}")

        // Optional: tool to export your presentation as PNGs and/or PDF.
...
The decoration can react to xref:slide.adoc#context-data[context data attached to the slide] being currently displayed.

To continue xref:slide.adoc#custom-context-data[the slide's example] of text banner, you can retrieve the slide's context data and react upon it:
...
Presentation(
    ...
    val bannerText = presentationState.currentSlide.context[Banner]?.text
    if (bannerText != null) {
